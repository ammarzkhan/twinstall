...
**Twinstall collects nothing, sends nothing, and has nowhere to send it to.**
...
Twinstall is transmitted anywhere, because the program has no means of transmitting anything.

That is a checkable claim rather than a promise. The whole source is in this repository, and
neither `Twinstall.Core`, `Twinstall.Platform` nor `Twinstall.App` references `System.Net`,
...
If your firewall ever shows Twinstall making a connection, that is a bug worth reporting — see
[SECURITY.md](SECURITY.md).

## What it stores, and where

Everything Twinstall writes lives under `%LOCALAPPDATA%\Twinstall` and in your own user section
of the registry. Nothing is written outside your Windows profile, and nothing needs an
administrator.

| Where | What |
|---|---|
| `%LOCALAPPDATA%\Twinstall\config.tsv` | the application you set up, its URL scheme, the names **you** chose for your accounts, their profile folder paths, badge colours |
| `%LOCALAPPDATA%\Twinstall\twinstall.log` | what it did and why — setup steps, and which account each sign-in link was routed to |
| `%LOCALAPPDATA%\Twinstall\icons\*.ico` | badged icons, composed at run time from the icon of the app already on your machine |
| `HKEY_CURRENT_USER` | the protocol handler registration, and the entry that makes Twinstall appear in Settings → Apps |
...
Routing a sign-in link is the one moment Twinstall handles something sensitive. The callback URL
...
Twinstall never sees your password. Signing in happens between your browser and the
application's own servers, exactly as it would without Twinstall installed. Its only job is
deciding *which window* the result lands in.

## What it reads

To route a link correctly, Twinstall asks Windows which copies of the target application are
...
Twinstall points the application at them and never reads their contents.

Uninstalling Twinstall deliberately leaves those folders alone, and tells you where they are so
...
**Settings → Apps → Installed apps → Twinstall → Uninstall**, like any other program. That
removes the executable, the shortcuts, the badged icons, the registry entries, and the log.

Inside the app, **Remove Twinstall's changes** does the same without uninstalling.

## Children

Twinstall is a desktop utility with no accounts, no content and no network access. It is not
directed at children and collects no information from anyone, of any age.

## Questions

Open an issue on [the repository](https://github.com/ammarzkhan/twinstall/issues). For anything
