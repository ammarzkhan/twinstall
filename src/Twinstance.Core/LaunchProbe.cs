...
namespace Twinstance.Core
{
    ...
    /// start and the polling loop are in Twinstance.Platform.
    /// </summary>
    public static class LaunchProbe
    {
        ...
        public static string ProbeDirectory(string tempRoot, string token)
        {
            ...
            return PathUtil.Join(tempRoot, "Twinstance", ProbeFolderPrefix + token);
        }
        ...
        public static string Explain(ProbeVerdict verdict, string exePath)
        {
            ...
            switch (verdict)
            {
                ...
                case ProbeVerdict.LaunchBlocked:
                    return "Windows would not let Twinstance start " + name + ", so it could not be tested. "
