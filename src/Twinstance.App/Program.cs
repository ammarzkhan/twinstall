        ...
        internal static int Main(string[] argv)
        {
            try
            {
                if (argv != null && argv.Length > 0)
                {
                    ...
                    // record an absolute path, so moving Twinstance — or setting it up from a
                    ...
                    {
                        Ui.Info("Twinstance " + typeof(Program).Assembly.GetName().Version);
                        return 0;
                    }
            ...
            catch (Exception ex)
            {
                ...
                Log.Write("FATAL: " + ex);
                Ui.Error("Twinstance hit an unexpected problem.\r\n\r\n" + ex.Message
                       + "\r\n\r\nDetails are in:\r\n" + AppPaths.LogFile);
                return 1;
            }
        }
