...
namespace Twinstance.App
{
    ...
    /// <summary>
    /// Everything Twinstance changes outside its own folder, in one place, so it can all be
    /// undone in one place.
    ///
    /// Protocol association goes through <c>RegisteredApplications</c> — a ProgId plus a
    /// UrlAssociations capability — and then the *user* chooses Twinstance in Settings. It
    ...
    {
        private const string CapabilitiesPath = @"Software\Twinstance\Capabilities";
        ...
        {
            return "Twinstance.Url." + (scheme ?? string.Empty).ToLowerInvariant();
        }
        ...
        internal static bool RegisterProtocol(string scheme)
        {
            ...
            try
            {
                ...
                {
                    prog.SetValue(null, "Twinstance link (" + scheme + ")");
        ...
        internal static void RemoveAllRegistration(AppConfig cfg)
        {
            ...
            try
            {
                ...
                using (RegistryKey sw = Registry.CurrentUser.OpenSubKey("Software", true))
                    sw?.DeleteSubKeyTree("Twinstance", throwOnMissingSubKey: false);
            }
        ...
        ///
        /// Measured on Windows 11, 7 Aug 2026, after choosing Twinstance in Settings:
        ///
        ///   ...\Shell\Associations\UrlAssociations\claude\UserChoiceLatest\ProgId  ProgId = Twinstance.Url.claude
        ///
        ...
        /// <summary>
        /// Opens Settings **on Twinstance's own Default-apps page**, where the scheme is the only
        ...
        /// </summary>
        internal const string SelfTestHost = "twinstance-selftest";
        ...
        internal static int CreateShortcuts(AppConfig cfg, bool alsoOnDesktop)
        {
            ...
            try
            {
                ...
                {
                    // In the Start menu they sit inside a Twinstance folder, so the account name
        ...
        /// <summary>
        /// Deletes only the shortcuts Twinstance itself made, identified by where they point
        ...
        private static void PruneOurShortcuts(Type shellType, object shell, string folder)
        {
            ...
            foreach (string path in links)
            {
                try
                {
                    ...
                    if (!string.Equals(Path.GetFileName(target ?? string.Empty), "Twinstance.exe",
        ...
        private static bool Write(Type shellType, object shell, string linkPath, Instance inst, string app)
        {
            ...
            Set(linkType, link, "Description", app + " — " + inst.Name + " (via Twinstance)");
        ...
        /// <summary>
        /// A Start-menu entry for Twinstance itself, distinct from the per-account ones. Without
        ...
        internal static void CreateAppShortcut()
        {
            try
            {
                ...
                Log.Write("added the Twinstance shortcut");
            }
        ...
        private const string WatcherValue = "Twinstance Icon Watcher";
        ...
        /// every Twinstance.exe would close the window the user is standing in.
        /// </summary>
        internal static void StopIconWatcher()
        {
            int self = Environment.ProcessId;
            foreach (Process p in Process.GetProcessesByName("Twinstance"))
            {
                try
                {
        ...
        private const string UninstallKey =
            @"Software\Microsoft\Windows\CurrentVersion\Uninstall\Twinstance";

        /// <summary>
        /// Puts Twinstance in Settings → Apps → Installed apps, so it can be removed the way
        ...
        internal static void RemoveUninstallEntry()
        {
            try
            {
                ...
                {
                    k?.DeleteSubKeyTree("Twinstance", throwOnMissingSubKey: false);
                }
            }
        ...
        /// <summary>
        /// Deletes Twinstance's own program folder after the process exits, since a running
        /// executable cannot delete itself.
        ///
        /// Guarded hard, because this ends in `rmdir /s /q`: it will only ever touch a folder
        /// named Twinstance directly beneath %LOCALAPPDATA%\Programs. Anywhere else — a build
        ...
        /// <summary>%LOCALAPPDATA%\Programs\Twinstance, and nothing else.</summary>
        ...
        internal static IList<string> DescribeChanges(AppConfig cfg, bool taskbarOptIn, bool desktopShortcuts)
        {
            var lines = new List<string>
            {
                ...
                "Register Twinstance as a candidate handler for " + (cfg?.Scheme ?? "(none)") + "://"
            };
            ...
            if (!string.IsNullOrWhiteSpace(cfg?.Scheme))
                lines.Add("Open Settings so you can choose Twinstance for " + cfg.Scheme + ":// yourself");
