    ...
    internal sealed class MainForm : Form
    {
        ...
        private void OnClosing(FormClosingEventArgs e)
        {
            ...
            DialogResult choice = Ui.Choose(
              ...
              + "wrong account, which is the problem Twinstance exists to fix.\r\n\r\n"
              + "Yes\t\tUndo everything Twinstance changed\r\n"
            ...
            UndoEverything();
            Ui.Info("Twinstance's changes have been removed.\r\n\r\n"
        ...
        private void BuildChooseApp()
        {
            ...
            {
                y = AddNote(y, "None of the apps Twinstance knows about were found on this PC. "
                             + "That is fine — any Chromium or Electron app works. Point it at the .exe.");
            }
            else
            {
                y = AddNote(y, "These are installed on this PC. Twinstance will start the one you pick, "
        ...
        private void BuildResult()
        {
            ...
            if (r != null)
            {
                ...
                if (!string.IsNullOrEmpty(r.Scheme))
                    y = AddFact(y, true, "Sign-in links use " + r.Scheme + ":// — Twinstance can route those");
                else
        ...
        private void BuildAccounts()
        {
            ...
            Say(_config.Instances.Count < 2
                ? "Add a second account — that is the point of Twinstance."
        ...
        private void BuildDone()
        {
            ...
            Head(handled ? "You're set up" : "One step left",
                 handled ? "All done" : "Twinstance is not handling links yet");
            ...
            if (!needsHandler)
            {
                if (handled) y = AddFact(y, true, "Twinstance is handling " + _config.Scheme + ":// links");
                ...
                AddLogButton(y + 52);
                Say("Everything Twinstance changed can be undone from the first screen.");
                return;
            }
        ...
        private void OpenDefaultsPage()
        {
            ...
            Say("Settings is open on Twinstance's page — do the two steps above, then come back.");
        }
        ...
        private async void GoNext()
        {
            switch (_step)
            {
                ...
                default:
                    ...
                    // "Choose Twinstance" (not set). It never offers to finish regardless.
        ...
        {
            if (!Ui.Confirm("Remove Twinstance's shortcuts, icons, registry entries and settings?\r\n\r\n"
