...
namespace Twinstance.App
{
    ...
    internal static class Router
    {
        ...
        internal static int Route(string url)
        {
            ...
            // setup. Reaching here means the user picked Twinstance and it works — say so, and
            ...
            {
                Log.Write("self-test link received — Twinstance is the handler");
                Ui.Info("That worked.\r\n\r\n"
                      + "Twinstance is now handling these links, so a sign-in will land on the "
                      + "account you started it from.\r\n\r\nYou can close this and Twinstance.");
                return 0;
            }
            ...
            if (string.IsNullOrWhiteSpace(cfg.ExePath) || !File.Exists(cfg.ExePath))
            {
                ...
                Ui.Error("Twinstance cannot find the application it was set up for.\r\n\r\n"
                       + "Open Twinstance and set it up again.");
                return 1;
            }

            // A missing or unreadable config must not strand the user mid sign-in. Handing the
            // link to the default profile is what would have happened without Twinstance at all.
        ...
        internal static int Watch(int seconds)
        {
            ...
            if (seconds <= 0)
            {
                using (var only = new System.Threading.Mutex(true, @"Local\TwinstanceIconWatcher", out bool mine))
                {
                    if (!mine)
                    {
            ...
            using (System.Drawing.Bitmap logo = LogoExtractor.Extract(cfg.ExePath))
            {
                foreach (Instance inst in cfg.Instances)
                {
                    ...
                    try
                    {
                        ...
                        // copied into Twinstance's folder. If it has since been moved or
        ...
        private static bool Start(string exePath, string dataDir, string url)
        {
            ...
            catch (System.ComponentModel.Win32Exception ex)
            {
                ...
                Ui.Error("Twinstance could not start the application.\r\n\r\n" + ex.Message);
                return false;
            }
